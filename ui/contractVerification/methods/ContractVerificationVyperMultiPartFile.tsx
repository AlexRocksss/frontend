...
const ContractVerificationVyperMultiPartFile = () => {
  ...
  const interfacesHint = (
    <>
      <span>{ t('Add any') }</span>
      <Link href="https://docs.vyperlang.org/en/stable/interfaces.html" target="_blank">{ t('required interfaces') }</Link>
      <span>{ t('for the main compiled contract') }</span>
    </>
  );

  return (
    <ContractVerificationMethod title={ t("Contract verification via Vyper (multi-part files)") }>
