...
const AssetFilter = ({ value = [], handleFilterChange }: Props) => {
  ...
  return (
    <TableColumnFilter
      title={ t('advancedFilter.filterAsset') }
    ...
    >
      <FilterInput
        ...
        placeholder={ t('advancedFilter.tokenNameOrSymbol') }
      ...
      { tokensQuery.data && !searchTerm && (
        <>
          <Text color="text.secondary" fontWeight="600" mt={ 3 }>{ t('advancedFilter.popular') }</Text>
      ...
      { searchTerm && tokensQuery.data && !tokensQuery.data?.items.length && <Text>{ t('advancedFilter.noTokensFound') }</Text> }
