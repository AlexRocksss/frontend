...
const ArbitrumL2TxnBatchesListItem = ({ item, isLoading }: Props) => {
  ...
  return (
    <ListItemMobileGrid.Container gridTemplateColumns="110px auto">
      ...
      <ListItemMobileGrid.Label isLoading={ isLoading }>{ t('txnBatches.parentStatusHeader', { parent: layerLabels.parent }) }</ListItemMobileGrid.Label>
      <ListItemMobileGrid.Value>
        <ArbitrumL2TxnBatchStatus status={ item.commitment_transaction.status } isLoading={ isLoading }/>
      </ListItemMobileGrid.Value>

      <ListItemMobileGrid.Label isLoading={ isLoading }>{ t('txnBatches.parentBlockHeader', { parent: layerLabels.parent }) }</ListItemMobileGrid.Label>
      <ListItemMobileGrid.Value>
        <BlockEntityL1
      ...
      { item.blocks_count && (
        <>
          <ListItemMobileGrid.Label isLoading={ isLoading }>{ t('txnBatches.blockCountHeader') }</ListItemMobileGrid.Label>
          <ListItemMobileGrid.Value>
      ...
      <ListItemMobileGrid.Label isLoading={ isLoading }>{ t('txnBatches.parentTxnHeader', { parent: layerLabels.parent }) }</ListItemMobileGrid.Label>
      <ListItemMobileGrid.Value>
        <TxEntityL1
      ...
      <ListItemMobileGrid.Label isLoading={ isLoading }>{ t('txnBatches.dataContainerHeader') }</ListItemMobileGrid.Label>
      <ListItemMobileGrid.Value>
