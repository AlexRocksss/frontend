...
import idMessage from '../messages/id.json';
import jaMessage from '../messages/ja.json';
import krMessage from '../messages/kr.json';
import msMessage from '../messages/ms.json';
import ruMessage from '../messages/ru.json';
import thMessage from '../messages/th.json';
...
function MyApp({ Component, pageProps }: AppPropsWithLayout) {
  ...
  useEffect(() => {
    ...
    const loadMessages = async(locale: string) => {
      if (locale === 'ja') {
        setMessages(jaMessage);
      } else if (locale === 'kr') {
        setMessages(krMessage);
      } else if (locale === 'ru') {
        setMessages(ruMessage);
      } else if (locale === 'th') {
        setMessages(thMessage);
      } else if (locale === 'id') {
        setMessages(idMessage);
      } else if (locale === 'vi') {
        setMessages(viMessage);
      } else if (locale === 'ms') {
        setMessages(msMessage);
      } else if (locale === 'zh') {
