...
| NEXT_PUBLIC_CUSTOM_AD_CONFIG | `{ ads: Array<{ id, image_url, image_url_dark?, image_url_mobile?, image_url_mobile_dark?, link_url, alt? }>, pages: { home?, tx?, address?, token?, marketplace? : { enabled: boolean, ad_ids?: Array<string> } } }` | Self-served ad inventory and per-page toggles. Only used when `NEXT_PUBLIC_AD_BANNER_PROVIDER=custom`. Image fallback chain: mobile_dark → mobile → dark → image_url. A page without an `ad_ids` list pulls from all `ads`; a page missing from `pages` shows no banner. | - | - | `{'ads':[{'id':'promo1','image_url':'https://cdn.example.com/promo1_desktop.png','image_url_mobile':'https://cdn.example.com/promo1_mobile.png','link_url':'https://example.com'}],'pages':{'home':{'enabled':true,'ad_ids':['promo1']},'tx':{'enabled':false}}}` | - |
